...
struct AddToLibrarySheet: View {
    ...
    // Mock collections data with the new properties
    ...
    private func CollectionRow(collection: Collection) -> some View {
        guard let collectionId = collection.id else { return AnyView(EmptyView()) }
        
        let isSelected = selectedCollections.contains(collectionId)
        let collectionColor = Color(hex: collection.color)
...
private struct NewCollectionView: View {
    ...
    private func createCollection() {
        ...
        do {
            try vm.addCollection(name: collectionName, color: selectedColor.hex, icon: selectedIcon)
